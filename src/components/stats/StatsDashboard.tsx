...
import { Award, Flame, BookOpen, Clock, Calendar, CheckCircle2, Zap } from 'lucide-react';
...
export const StatsDashboard: React.FC<StatsDashboardProps> = ({ progress }) => {
  ...
  // Generate 28-day Heatmap Grid Data
  const heatmapDays = Array.from({ length: 28 }, (_, i) => {
    const intensity = (i * 7 + 3) % 5; // 0: none, 1-4: increasing activity
    ...
    <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5 pb-24">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-card border border-slate-200/80 dark:border-slate-700">
          ...
          <p className="text-2xl font-black text-slate-900 dark:text-slate-100">{accuracy}%</p>
          <span className="text-[10px] text-slate-400 font-semibold">High Recognition</span>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-card border border-slate-200/80 dark:border-slate-700">
          ...
          <span className="text-[10px] text-orange-500 font-semibold">🔥 Personal Best</span>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-card border border-slate-200/80 dark:border-slate-700">
          ...
          <p className="text-2xl font-black text-slate-900 dark:text-slate-100">4.2 hrs</p>
          <span className="text-[10px] text-purple-500 font-semibold">This Month</span>
        </div>
      </div>
